...
## What Resources will Gets Created

- **26 IAM Users** with console access
- **3 IAM Groups** (Education, Managers, Accounting)
- **Group Memberships** based on user attributes
- **User Tags** with metadata (DisplayName, Department, JobTitle, mobile_number, employee_id, email, office_location, hire_year)
...
```powershell
aws s3 mb s3://demo-terraform-state-file-1110001 --region us-east-1
...
├── mfa.tf              # Enforce user to enable MFA, unless no access
...
### Accounting Group
- Kevin Malone (kmalone)
- Angela Martin (amartin)
- Oscar Martinez (omartinez)
